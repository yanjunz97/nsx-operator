...
func NewVirtualServerService(client *l4.Client) (*VirtualServerService, error) {
	wg := sync.WaitGroup{}
	wgDone := make(chan bool)
	fatalErrors := make(chan error)

	vsc := &VirtualServerService{
		Client: client,
		Store: &VirtualServerStore {
	...
	wg.Add(1)
	go vsc.InitializeResourceStore(&wg, fatalErrors, "virtualServer", nil, vsc.store)
	go func() {
		wg.Wait()
		close(wgDone)
	}()
...
func (s *VirtualServerService) CreateLbVsForPort(lbVirtualServer model.LBVirtualServer) error {
	
	// check lbs limit by registering to lbs pool memeber reg

	// call sdk api to create lbvs
	s.Client.VirtualServerClient.Patch(*lbVirtualServer.Id, lbVirtualServer)
...
func (s *VirtualServerService) DeleteLbVsForPort(lbVirtualServerId string) error {
	// set delete if transaction exists
	// otherwise delete with retry

	// use MarkedForDelete for here?
	return s.Client.VirtualServerClient.Delete(lbVirtualServerId, false)
}

func (s *VirtualServerService) UpdateLbVsAttributes(virtualServerId string, lbVirtualServer model.LBVirtualServer) (model.LBVirtualServer, error) {
	// get PoolId from store: exist_lb_pool = self._lb_vs_store.get(vs_id).pool_id
	return s.Client.VirtualServerClient.Update(virtualServerId, lbVirtualServer)
}
