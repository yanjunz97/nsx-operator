...
func TestPodReconciler_getSubnetByIP(t *testing.T) {
	r := &PodReconciler{
		SubnetService: &subnet.SubnetService{},
	}
	...
	subnetPath, err := r.getSubnetByIP(pod, "subnetset-1")
	assert.Nil(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			prepareFunc: func(t *testing.T, pr *PodReconciler) *gomonkey.Patches {
				...
				patches.ApplyFunc((*PodReconciler).getSubnetByIP, func(r *PodReconciler, pod *v1.Pod, subnetSetUID string) (string, error) {
